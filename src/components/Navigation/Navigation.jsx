import { Link, useLocation } from "react-router-dom"
import House from "../../assets/Icon/House"
import TrophyLead from "../../assets/Icon/Trophy"
import Earns from "../../assets/Icon/Earn"
import Frens from "../../assets/Icon/Frens"
import Shield from "../../assets/Icon/Shield"

const activeColor = "#0077b3"
const inactiveColor = "#757575"
const textColor = "text-gray-300 hover:text-white"
const baseButtonStyles = "bg-transparent flex flex-col items-center gap-1 focus:outline-none focus:ring-2 focus:ring-[#0077b3]"

const navItems = [
  {
    path: "/home",
    label: "Главная",
    icon: House,
    iconProp: "color",
  },
  {
    path: "/leaders",
    label: "Лидеры",
    icon: TrophyLead,
    iconProp: "color",
  },
  {
    path: "/squads",
    label: "Сквады",
    icon: Shield,
    iconProp: "imageHref",
    inactiveImage: "data:image/png;base64,",
    activeImage: "data:image/png;base64,",
  },
  {
    path: "/frens",
    label: "Друзья",
    icon: Frens,
    iconProp: "color",
  },
  {
    path: "/earn",
    label: "Задания",
    icon: Earns,
    iconProp: "color",
  },
]

export const Navigation = () => {
  const { pathname } = useLocation()
  
  return (
    <div className='flex-grow flex items-center w-full text-sm mt-4 border-t-2 border-[#151515]'>
      <div className='w-full bg-[#131313] pb-7 pt-4 flex justify-around navigate-menu-hover'>
        {navItems.map((item) => {
          const isActive = pathname === item.path
          const iconProps = {
            size: 30,
            className: 'ml-0 mb-1 inline-block',
            [item.iconProp]: isActive 
              ? item.iconProp === "imageHref" 
                ? item.activeImage 
                : activeColor 
              : item.iconProp === "imageHref" 
                ? item.inactiveImage 
                : inactiveColor,
          }

          return (
            <Link 
              key={item.path}
              to={item.path} 
              className="focus:outline-none"
            >
              <button 
                className={`${baseButtonStyles} ${
                  isActive ? 'text-[#0077b3]' : textColor
                }`}
                style={{
                  WebkitTapHighlightColor: 'transparent'}}
              >
                <item.icon {...iconProps} />
                <span className="font-light text-sm text-[#808080]">
                  {item.label}
                </span>
              </button>
            </Link>
          )
        })}
      </div>
    </div>
  )
}